    ...
    internal abstract partial class CustomComboFunctions
    {
        ...
        */
        ...
        public static unsafe class ItemManager
        {
            ...
            private static Dictionary<uint, (uint status, DateTime lastChecked)> _actionStatusCache = [];

            // Cache
            static ItemManager() => _actionManagerInstance = ActionManager.Instance();
            ...
            public static bool CanUseItem(ActionType actionType, uint actionID)
            {
                ...
                if (_actionStatusCache.TryGetValue(actionID, out (uint status, DateTime lastChecked) cache) && (DateTime.Now - cache.lastChecked).TotalMilliseconds < 500)
                {
            ...
            public static bool UseItem(uint itemID) => _actionManagerInstance->UseAction(ActionType.Item, itemID, 0xFFFF, 65535);
        }

        public static unsafe bool UsePotion(uint itemID)
        {
            if (!ItemManager.CanUseItem(ActionType.Item, itemID))
                return false;

            // Evaluate weaving conditions only when necessary
            bool canWeaveWeaponskill = CanWeave();
            bool canWeaveSpell = CanSpellWeave();
        ...
        public static unsafe uint GetItemStatus(uint itemID) =>
            ...
            ItemManager._actionManagerInstance is null ? uint.MaxValue : ItemManager._actionManagerInstance->GetActionStatus(ActionType.Item, itemID);
