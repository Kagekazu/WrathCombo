...
internal partial class RDM
{
    ...
    private static bool TryLucidDreaming(uint actionID, int MPThreshold, uint lastComboMove) => All.CanUseLucid(actionID, MPThreshold)
            && InCombat()
...
#pragma warning restore IDE0042
    ...
    internal static class RDMOpenerLogic
    {
        ...
        private static bool DoPrePullSteps(ref uint actionID)
        {
            ...
            {
                if (LocalPlayer?.CastActionId == Veraero3 && PrePullStep == 1)
        ...
        private static bool DoOpener(ref uint actionID)
        {
            ...
            {
                if (WasLastAction(Verthunder3) && OpenerStep == 1)
                    OpenerStep++;
                else if (OpenerStep == 1)
                    actionID = Verthunder3;

                if (WasLastAction(All.Swiftcast) && OpenerStep == 2)
                    OpenerStep++;
                else if (OpenerStep == 2)
                    actionID = All.Swiftcast;

                if (WasLastAction(Verthunder3) && OpenerStep == 3)
                    OpenerStep++;
                else if (OpenerStep == 3)
                    actionID = Verthunder3;

                if (WasLastAction(Fleche) && OpenerStep == 4)
                    OpenerStep++;
                else if (OpenerStep == 4)
                    actionID = Fleche;

                if (WasLastAction(Acceleration) && OpenerStep == 5)
                    OpenerStep++;
                else if (OpenerStep == 5)
                    actionID = Acceleration;

                if (WasLastAction(Verthunder3) && OpenerStep == 6)
                    OpenerStep++;
                else if (OpenerStep == 6)
                    actionID = Verthunder3;

                if (WasLastAction(Embolden) && OpenerStep == 7)
                    OpenerStep++;
                else if (OpenerStep == 7)
                    actionID = Embolden;

                if (WasLastAction(Manafication) && OpenerStep == 8)
                    OpenerStep++;
                else if (OpenerStep == 8)
                    actionID = Manafication;

                if (WasLastAction(EnchantedRiposte) && OpenerStep == 9)
                    OpenerStep++;
                else if (OpenerStep == 9)
                    actionID = EnchantedRiposte;

                if (WasLastAction(ContreSixte) && OpenerStep == 10)
                    OpenerStep++;
                else if (OpenerStep == 10)
                    actionID = ContreSixte;

                if (WasLastAction(EnchantedZwerchhau) && OpenerStep == 11)
                    OpenerStep++;
                else if (OpenerStep == 11)
                    actionID = EnchantedZwerchhau;

                if (WasLastAction(Engagement) && OpenerStep == 12)
                    OpenerStep++;
                else if (OpenerStep == 12)
                    actionID = Engagement;

                if (WasLastAction(EnchantedRedoublement) && OpenerStep == 13)
                    OpenerStep++;
                else if (OpenerStep == 13)
                    actionID = EnchantedRedoublement;

                if (WasLastAction(Corpsacorps) && OpenerStep == 14)
                    OpenerStep++;
                else if (OpenerStep == 14)
                    actionID = Corpsacorps;

                if (WasLastAction(Verholy) && OpenerStep == 15)
                    OpenerStep++;
                else if (OpenerStep == 15)
                    actionID = Verholy;

                if (WasLastAction(ViceOfThorns) && OpenerStep == 16)
                    OpenerStep++;
                else if (OpenerStep == 16)
                    actionID = ViceOfThorns;

                if (WasLastAction(Scorch) && OpenerStep == 17)
                    OpenerStep++;
                else if (OpenerStep == 17)
                    actionID = Scorch;

                if (WasLastAction(Engagement) && OpenerStep == 18)
                    OpenerStep++;
                else if (OpenerStep == 18)
                    actionID = Engagement;

                if (WasLastAction(Corpsacorps) && OpenerStep == 19)
                    OpenerStep++;
                else if (OpenerStep == 19)
                    actionID = Corpsacorps;

                if (WasLastAction(Resolution) && OpenerStep == 20)
                    OpenerStep++;
                else if (OpenerStep == 20)
                    actionID = Resolution;

                if (WasLastAction(Prefulgence) && OpenerStep == 21)
                    OpenerStep++;
                else if (OpenerStep == 21)
                    actionID = Prefulgence;

                if (WasLastAction(GrandImpact) && OpenerStep == 22)
                    OpenerStep++;
                else if (OpenerStep == 22)
                    actionID = GrandImpact;

                if (WasLastAction(Acceleration) && OpenerStep == 23)
                    OpenerStep++;
                else if (OpenerStep == 23)
                    actionID = Acceleration;

                if (WasLastAction(Verfire) && OpenerStep == 24)
                    OpenerStep++;
                else if (OpenerStep == 24)
                    actionID = Verfire;

                if (WasLastAction(GrandImpact) && OpenerStep == 25)
                    OpenerStep++;
                else if (OpenerStep == 25)
                    actionID = GrandImpact;

                if (WasLastAction(Verthunder3) && OpenerStep == 26)
                    OpenerStep++;
                else if (OpenerStep == 26)
                    actionID = Verthunder3;

                if (WasLastAction(Fleche) && OpenerStep == 27)
                    OpenerStep++;
                else if (OpenerStep == 27)
                    actionID = Fleche;

                if (WasLastAction(Veraero3) && OpenerStep == 28)
                    OpenerStep++;
                else if (OpenerStep == 28)
                    actionID = Veraero3;

                if (WasLastAction(Verfire) && OpenerStep == 29)
                    OpenerStep++;
                else if (OpenerStep == 29)
                    actionID = Verfire;

                if (WasLastAction(Verthunder3) && OpenerStep == 30)
                    OpenerStep++;
                else if (OpenerStep == 30)
                    actionID = Verthunder3;

                if (WasLastAction(Verstone) && OpenerStep == 31)
                    OpenerStep++;
                else if (OpenerStep == 31)
                    actionID = Verstone;

                if (WasLastAction(Veraero3) && OpenerStep == 32)
                    OpenerStep++;
                else if (OpenerStep == 32)
                    actionID = Veraero3;

                if (WasLastAction(All.Swiftcast) && OpenerStep == 33)
                    OpenerStep++;
                else if (OpenerStep == 33)
                    actionID = All.Swiftcast;

                if (WasLastAction(Veraero3) && OpenerStep == 34)
                    OpenerStep++;
                else if (OpenerStep == 34)
                    actionID = Veraero3;

                if (WasLastAction(ContreSixte) && OpenerStep == 35)
